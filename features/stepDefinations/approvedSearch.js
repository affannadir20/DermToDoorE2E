...
Before({tags: "@SearchApproved"}, async function () {
...
After({tags: "@SearchApproved"}, async function () {
    await new Promise(resolve => setTimeout(resolve, 4000));
    await browser.close();
});

Given('the user is logged into', async function () {
...
When('the user clicks on Approved button', { timeout: 20000 }, async function () {
    await searchPage.clickApprovedButton();
});

When('the user enters {string} as search parameter', { timeout: 20000 }, async function (searchQuery) {
    await searchPage.performSearch(searchQuery);
});

When('the user click on Search button', { timeout: 20000 }, async function () {
